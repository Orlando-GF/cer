...
export function VagasAtivasList({ vagas, onRemove }: VagasAtivasListProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-6">
      {vagas.map((vaga) => (
        <div key={vaga.id} className="group relative bg-slate-50 border border-slate-100 p-4 transition-all hover:border-primary/20 hover:shadow-md">
          <div className="flex justify-between items-start mb-3">
            <div className="flex items-center gap-2">
              <Badge className="bg-clinico-100 text-clinico-900 border-none rounded-none font-bold text-[9px] uppercase tracking-wider">
          ...
          <div className="space-y-2">
            <div className="flex items-start gap-3">
              <div className="w-8 h-8 rounded-none bg-white border border-slate-200 flex items-center justify-center shrink-0">
