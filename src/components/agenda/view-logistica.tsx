...
import { buscarAgendaLogistica } from "@/app/actions"
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-5 rounded-xl border shadow-sm">
        <div className="flex items-center gap-4">
          <div className="space-y-1">
            <span className="text-xs font-semibold text-slate-500 tracking-wider">Transporte da data</span>
            <Input 
              type="date" 
              className="w-full"
        ...
        <div className="md:col-span-3">
          <Card className="border-none shadow-sm overflow-hidden bg-white">
            <Table>
              ...
              <TableBody>
                {loading ? (
                  <TableRow><TableCell colSpan={4} className="text-center py-12">Carregando rota...</TableCell></TableRow>
                ) : sessões.length === 0 ? (
                  <TableRow><TableCell colSpan={4} className="text-center py-12 text-slate-500">Nenhum paciente necessita de transporte hoje.</TableCell></TableRow>
                ) : (
                  sessões.map((sessao: any) => (
                    <TableRow key={sessao.id} className="hover:bg-slate-50/50">
                      ...
                      <TableCell>
                        <div className="flex flex-col">
                          <span className="font-bold text-slate-900">{sessao.paciente_nome}</span>
                          <span className="text-xs text-slate-500 flex items-center gap-1 mt-1">
                            <MapPin className="h-3 w-3" /> 
                            {sessao.paciente_logradouro || 'Endereço não informado'}, {sessao.paciente_numero} - {sessao.paciente_bairro}
                          </span>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex flex-wrap gap-1">
                          {sessao.tags_acessibilidade?.map((tag: string) => (
                            <Badge key={tag} className="bg-red-50 text-red-700 border-red-100 text-[9px] px-1.5 py-0">
                              {tag}
                            </Badge>
                          ))}
                          {(!sessao.tags_acessibilidade || sessao.tags_acessibilidade.length === 0) && (
                            <span className="text-[10px] text-slate-500">Sem restrições</span>
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex flex-col">
                            <span className="text-xs font-medium text-slate-700">{sessao.profissional_nome}</span>
                            <span className="text-[10px] text-slate-500">{sessao.especialidade_nome}</span>
                          </div>
                        </div>
                      </TableCell>
                    </TableRow>
          ...
          <div className="p-4 bg-amber-50 rounded-xl border border-amber-100 space-y-3">
             ...
             <div className="space-y-2">
                <div className="flex justify-between items-center text-xs">
                  ...
                  <Badge className="bg-amber-200 text-amber-900 border-none">
                    {sessões.filter((s: any) => s.tags_acessibilidade?.includes('Cadeirante')).length}
                  </Badge>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="text-amber-800">Uso de Oxigênio</span>
                  <Badge className="bg-amber-200 text-amber-900 border-none">
                    {sessões.filter((s: any) => s.tags_acessibilidade?.includes('Uso de Oxigénio')).length}
                  </Badge>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="text-amber-800">Acamados/Maca</span>
                  <Badge className="bg-amber-200 text-amber-900 border-none">
                    {sessões.filter((s: any) => s.tags_acessibilidade?.includes('Acamado/Uso de Maca')).length}
                  </Badge>
                </div>
             </div>
          </div>
          
          <Button variant="outline" className="w-full justify-start gap-2 h-12 text-slate-600 border-slate-200">
